...
- **GTM** container `GTM-KM56HHS2`, loaded first-party through the Google tag gateway measurement path `/wro1/gtm.js`. Cloudflare handles that path at the edge; `src/worker.ts` never sees it.
...
- **Contact form events** are pushed from `src/components/ContactForm.astro`, all of them carrying `form_name: 'contact'`:
  ...
  - **`form_error`** on any rejected submit, with `error_type` (`validation`, `bot_check` or `server`), `error_code`, `error_reason` (the server's own message for non-validation errors, such as `Failed to send email (E_SENDER_NOT_VERIFIED)`, so GA4 can tell a mail outage from a bot block) and `error_fields` - a comma-joined list of field NAMES only. No submitted value ever reaches the dataLayer.
...
- **Adding or renaming a dataLayer event is a GTM change too.** `tests/e2e/gtm-contract.spec.ts` reads the published container and fails CI until a GA4 tag fires on the new name and forwards its keys, or the event is listed in `UNTRACKED_EVENTS` in `tests/e2e/gtm.ts` with a reason. Publish the GTM change first; a trigger can listen for old and new names during the switch.
